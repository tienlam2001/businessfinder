...
import { Printer, Trash2, Edit, User, Home, Phone, Briefcase, Link, Users, Image, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
export default function PersonProfile({ data, onDelete, onEdit }) {
  ...
  const handlePrint = useReactToPrint({ content: () => printRef.current, documentTitle: `${data.name}_Profile` });
    ...
    <div className="glass-card screen-only">
      ...
      <div className="print-only">
        <div ref={printRef} className="print-container" style={{ padding: '40px', color: 'black', background: 'white', fontFamily: 'Arial, sans-serif' }}>
            <h1 style={{ borderBottom: '3px solid #333', paddingBottom: '10px', marginBottom: '20px' }}>{data.name}</h1>
            <p><strong>Age:</strong> {data.age || 'N/A'}</p>
            
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px', margin: '40px 0' }}>
                <div>
                    <h3 style={{ background: '#eee', padding: '10px' }}>Contact Information</h3>
                    <p><strong>Home Address:</strong><br/>{data.homeAddress || 'N/A'}</p>
                    <p><strong>Phone Numbers:</strong> {data.phones?.join(', ') || 'N/A'}</p>
                </div>
                <div>
                     <h3 style={{ background: '#eee', padding: '10px' }}>Associations</h3>
                     <p><strong>Partners:</strong> {data.partners?.join(', ') || 'N/A'}</p>
                     <p><strong>Businesses / LLCs:</strong> {data.businesses?.join(', ') || 'N/A'}</p>
                     <p><strong>Properties:</strong> {data.properties?.join(', ') || 'N/A'}</p>
                </div>
            </div>

            {data.socials && data.socials.length > 0 && (
              <div>
                <h3 style={{ background: '#eee', padding: '10px' }}>Social Media</h3>
                <ul>
                  {data.socials.map((s, i) => <li key={i}><strong>{s.platform}:</strong> {s.url}</li>)}
                </ul>
              </div>
            )}
